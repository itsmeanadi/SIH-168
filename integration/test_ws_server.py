...
def test_non_object_json_safely_rejected():
    server = TelemetryServer(MockWrapper())
    server.handle_raw_message("[1, 2, 3]")
    server.handle_raw_message('"string"')
    server.handle_raw_message("123")
    assert server.wrapper.imu_count == 0
...
def test_malformed_gnss_data_rejected():
    ...
    server.handle_raw_message('{"type": "gnss", "timestamp": 1700000000.0, "longitude": 77.41}')
    assert server.wrapper.last_gnss is None

    server.handle_raw_message('{"type": "gnss", "timestamp": 1700000000.0, "latitude": 23.25}')
    assert server.wrapper.last_gnss is None

    # Non-numeric latitude/longitude
    server.handle_raw_message('{"type": "gnss", "timestamp": 1700000000.0, "latitude": "north", "longitude": 77.41}')
    assert server.wrapper.last_gnss is None

    # Non-numeric speed/heading/accuracy
    server.handle_raw_message('{"type": "gnss", "timestamp": 1700000000.0, "latitude": 23.25, "longitude": 77.41, "speed": "fast"}')
    assert server.wrapper.last_gnss is None

    # Boolean values rejected
    server.handle_raw_message('{"type": "gnss", "timestamp": 1700000000.0, "latitude": true, "longitude": 77.41}')
    assert server.wrapper.last_gnss is None
